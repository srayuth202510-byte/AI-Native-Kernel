...
async fn filters_exclude_unwanted_intents() {
    ...
    let event_intent = Intent::new("evt-1", IntentType::Event, "heartbeat", IntentPriority::Low, "system");
    let cmd_intent = Intent::new("cmd-1", IntentType::Command, "spawn-agent", IntentPriority::High, "user");

    assert!(!bus.passes_filters(&event_intent).await, "event should be filtered out");
    assert!(bus.passes_filters(&cmd_intent).await, "command should pass filter");
}
...
async fn multiple_filters_and_priority() {
    ...
    let ok_intent = Intent::new("ok", IntentType::Structured, "data", IntentPriority::High, "agent");
    let fail_intent = Intent::new("fail", IntentType::Structured, "data", IntentPriority::Low, "agent");

    assert!(bus.passes_filters(&ok_intent).await);
    assert!(!bus.passes_filters(&fail_intent).await, "low priority structured should be filtered");
}
...
async fn disabled_filters_ignored() {
    ...
    let intent = Intent::new("evt", IntentType::Event, "payload", IntentPriority::Low, "test");
    assert!(bus.passes_filters(&intent).await, "disabled filter should be ignored");
}
...
async fn source_target_filters() {
    ...
    let mut intent = Intent::new("check", IntentType::Command, "do-something", IntentPriority::Medium, "trusted-agent");
    intent.target = Some("worker-pool".to_string());

    assert!(bus.passes_filters(&intent).await);

    intent.source = "malicious-bot".to_string();
    assert!(!bus.passes_filters(&intent).await, "untrusted source should fail filter");
}
