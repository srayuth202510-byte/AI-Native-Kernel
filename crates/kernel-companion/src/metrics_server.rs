...
pub async fn start_metrics_server(addr_str: &str, cancel: CancellationToken) -> Result<()> {
    let _ = crate::observability::kernel_metrics();
...
mod tests {
    ...
    async fn test_metrics_server_lifecycle_and_response() {
        ...
        crate::observability::kernel_metrics().record_lsm_decision("allow", "allowlist");

        // เริ่มต้นเซิร์ฟเวอร์
        let server_addr = addr_str.clone();
        let server_cancel = cancel.clone();
        let server =
            tokio::spawn(async move { start_metrics_server(&server_addr, server_cancel).await });
        ...
        assert!(response.contains("ank_lsm_policy_decisions_total"));
