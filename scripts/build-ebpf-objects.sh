...
if [[ "$BPFTOOL_BIN" == /* ]]; then
    if [[ ! -x "$BPFTOOL_BIN" ]]; then
        echo "bpftool is not executable: $BPFTOOL_BIN" >&2
        exit 1
    fi
else
    if ! BPFTOOL_BIN="$(command -v "$BPFTOOL_BIN" 2>/dev/null)"; then
        echo "bpftool not found: ${BPFTOOL_BIN}" >&2
        exit 1
    fi
fi

if [[ ! -f "$VMLINUX_BTF" ]]; then
    echo "Missing kernel BTF: $VMLINUX_BTF" >&2
    exit 1
fi

if [[ ! -f "$BPF_HELPERS_H" ]]; then
    echo "Missing BPF helper headers: $BPF_HELPERS_H" >&2
    exit 1
fi
