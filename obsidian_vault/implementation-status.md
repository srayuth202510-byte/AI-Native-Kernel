...
Last verified: 2026-07-11 — **519 tests pass** (4 ignored, Qdrant-backed — now also run against a real Qdrant in CI), clippy clean, cargo audit clean. Hardening backlog H1–H8 complete (see §"Hardening Round 2026-07-10"); H1/H2/H3/H8 + H6/H7 validated end-to-end — H3 v2 multi-prefix scope + ank-run smoke test ผ่านบน kernel จริง 2026-07-11.
...
- **[H3] intent → scope compiler** — `kernel-companion/src/scope.rs`: compile token caps (grant) + intent metadata (narrow เท่านั้น) → operation-class flags + path prefixes, push ลง BPF maps ก่อน agent เริ่มงาน; kernel บังคับ path ด้วย `bpf_d_path`. **v2 multi-prefix (2026-07-11)**: map `pid_path_prefixes` เก็บได้สูงสุด 8 prefixes ต่อ PID (`scope_path` metadata = newline-separated, skill manifest ใช้ `scope_paths = [...]`) — path ผ่านถ้าอยู่ใต้ prefix ใดก็ได้ ทำให้ ank-run รัน dynamically-linked binary ใต้ path scope ได้ (เติม `/usr,/lib,/lib64,/etc/ld.so.cache` อัตโนมัติ, ปิดด้วย `--no-system-paths`). **✅ validated end-to-end 2026-07-11** (in-scope allow / second-prefix allow / out-of-scope deny / exec deny + ank-run smoke test: `cat /srv/ank-demo/hello.txt` ผ่าน, `cat /etc/shadow` โดน EPERM). BPF gotchas: hook ต้องใช้ `BPF_PROG(...)`, buffer ของ `bpf_d_path` ต้อง zero-init, ห้าม byte-compare loop แบบ early-exit ข้าม candidate (verifier state ระเบิด >1M insns) — ใช้ 8-byte u64 chunk compare + haszero trick แทน; และ BPF object ฝังใน binary — regen prebuilt-bpf แล้วต้อง rebuild ank-run ด้วย.
